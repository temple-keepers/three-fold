...
export default function AdminConsolePage() {
  ...
  async function checkAuth() {
    ...
    // Use own-profile policy (auth.uid() = id) — no recursion
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#F4F1EA' }}>
      {/* Sidebar */}
      <aside className="flex-shrink-0 flex flex-col h-screen sticky top-0 overflow-y-auto" style={{ width: 240, background: '#0F1E2E' }}>
        <div className="flex items-center gap-3 px-4 py-5 border-b" style={{ borderColor: '#1A2D40' }}>
        ...
        <nav className="flex-1 px-2 py-3">
          {TABS.map((t) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl mb-0.5 border-none cursor-pointer text-left"
              style={{
        ...
        <div className="px-3 pb-4 border-t pt-3" style={{ borderColor: '#1A2D40' }}>
          <Link href="/dashboard" className="flex items-center gap-2 px-3 py-2 rounded-xl no-underline hover:opacity-80" style={{ color: '#5A6B7A', fontSize: 12, fontFamily: 'DM Sans, sans-serif' }}>
            ← Back to app
          </Link>
        </div>
      </aside>
...
function OverviewTab({ stats, loading }: { stats: Stats | null; loading: boolean }) {
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="text-2xl font-medium mb-1" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#0F1E2E' }}>
        ...
        {metrics.map((m) => (
          <AdminCard key={m.label}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: `${m.color}12` }}>
                {m.icon}
              </div>
              <div>
                <div className="text-2xl font-bold" style={{ fontFamily: 'DM Sans, sans-serif', color: '#0F1E2E' }}>{m.value}</div>
                <div className="text-xs" style={{ color: '#8A9BAA', fontFamily: 'DM Sans, sans-serif' }}>{m.label}</div>
              </div>
            </div>
          </AdminCard>
        ))}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <button
              key={a.label}
              onClick={() => {
                // Need to dispatch to parent — handled via window event or state
                const event = new CustomEvent('admin-nav', { detail: a.tab });
                window.dispatchEvent(event);
              }}
              className="p-4 rounded-xl border-none cursor-pointer text-left"
              style={{ background: '#FAF8F4', border: '1px solid #E0DCD4', fontFamily: 'DM Sans, sans-serif', transition: 'all 0.15s ease' }}
              onMouseEnter={(e) => { (e.target as HTMLElement).style.borderColor = '#C7A23A'; }}
              onMouseLeave={(e) => { (e.target as HTMLElement).style.borderColor = '#E0DCD4'; }}
            >
              <span className="text-xl mb-1 block">{a.icon}</span>
              <div className="text-sm font-semibold" style={{ color: '#0F1E2E' }}>{a.label}</div>
              <div className="text-xs mt-0.5" style={{ color: '#8A9BAA' }}>{a.desc}</div>
            </button>
          ))}
        </div>
      </AdminCard>
